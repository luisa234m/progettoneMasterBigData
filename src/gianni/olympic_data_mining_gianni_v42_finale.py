# -*- coding: utf-8 -*-
"""olympic_data_mining_gianni_v42_finale.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1indR7CNR_sIBaGFPpRQ21_jT9KhU46at
...
Il dataset viene caricato con `pd.read_csv()` dal file 'dataset_olimpiadi_indicatori_ita.csv' e salvato in un DataFrame di pandas chiamato `df`.
...
# Get unique Codice_NOC and Nazione pairs and sort by Codice_NOC
paesi_e_codici = df[['Codice_NOC', 'Nazione']].drop_duplicates().sort_values(by='Codice_NOC')
#Ccntrollo visivo se ci sono NAzioni associate a codici_noc c con descrizione diversa, es: China e People's Republic of China
...
# ── Uniformazione nomi Nazione per stesso Codice_NOC ──

# CASO 1 — Varianti tipografiche dello stesso Paese: si uniformano in sicurezza.
# Mappa: Codice_NOC -> nome canonico da usare per tutte le sue righe.
nomi_canonici = {
    'HKG': 'Hong Kong',
...
for noc, nome in nomi_canonici.items():
    df.loc[df['Codice_NOC'] == noc, 'Nazione'] = nome

# CASO 2 — BRN: due Paesi DIVERSI sotto lo stesso codice (errore nei dati).
# Brunei fino al 2008, Bahrain dal 2012. Si separano assegnando il codice corretto.
df.loc[(df['Codice_NOC'] == 'BRN') & (df['Nazione'] == 'Brunei Darussalam'), 'Codice_NOC'] = 'BRU'
# (ora BRN resta solo Bahrain, BRU è Brunei)

# CASO 3 — RUS: URSS e Russia sono entità storiche diverse.
# Scelta consapevole: si mantengomno distinte (Soviet Union fino al 1991, Russian Federation dopo).
# Nessuna modifica: si lascia com'è, ma lo dichiaro nella relazione.

# ── Verifica finale ──
residui = df.groupby('Codice_NOC')['Nazione'].nunique()
residui = residui[residui > 1]
print("Codici con più nomi dopo la pulizia:", list(residui.index) if len(residui) else "solo RUS (voluto)")
...
# 2. Interpolazione limitata (Safety First!)
...
Il passaggio da 3368 a 2628 righe corrisponde alla conservazione di circa l'**80% del dataset originale**, eliminando solo le osservazioni le cui informazioni mancanti non erano recuperabili in modo affidabile.

Questo risultato conferma l'efficacia della strategia di pulizia "intelligente" adottata, basata su tre livelli successivi:

1. **interpolazione limitata** (massimo 3 anni di buco per Nazione), per non inventare trend artificiali su gap troppo ampi
2. **imputazione mediana per gruppo Nazione**, più robusta della media globale rispetto a outlier e asimmetrie
3. **eliminazione mirata** delle sole righe prive dei pilastri fondamentali (PIL, Popolazione, Score Medaglie)

---

### Implicazioni per la modellazione

- **Maggiore potenza statistica**: con oltre 2600 osservazioni la base dati è sensibilmente più robusta rispetto a un approccio di pulizia aggressivo (es. `dropna()` su tutte le colonne), che avrebbe ridotto il campione a poche centinaia di righe, aumentando il rischio di overfitting nei modelli successivi.
- **Maggiore rappresentatività**: un campione più ampio garantisce che il Random Forest e l'analisi SHAP successivi si basino su una varietà più ampia di Nazioni e archi temporali, rendendo i risultati più generalizzabili.

La scelta di non eliminare aggressivamente le righe con dati mancanti, preferendo invece un'imputazione conservativa e tracciabile, rappresenta quindi un compromesso metodologico esplicito tra qualità e quantità dei dati disponibili.
...
La fase di preparazione dei dati ha prodotto una matrice finale di dimensione pari a 2.628 osservazioni e 4 feature, rappresentando un dataset sufficientemente ampio e strutturato per l’applicazione di algoritmi di clustering.
...
- **Deviazione standard pari a circa 1**
...
### 2. Stabilità del punteggio per k = 3 e k = 4
...
si osserva una riduzione minima del punteggio rispetto al massimo.

➡️ Interpretazione:
l’aggiunta di cluster consente una maggiore granularità senza compromettere in modo significativo la qualità della separazione.
...
- **k ≥ 5**, il punteggio diminuisce in modo più evidente
...
- il Silhouette Score conferma che:
  - k = 3 mantiene alta qualità
  - senza perdita significativa rispetto a k = 2
...
- qualità statistica (Silhouette Score elevato)
...
# Prepare options for the 'Anno' (Year) dropdown
year_options = [None] + sorted(df_analisi['Anno'].astype(int).unique().tolist())

# Prepare options for the 'Nazione' (Country) dropdown
nation_options = [None] + sorted(df_analisi['Nazione'].unique().tolist())

# Create interactive selection points for 'Anno' and 'Nazione'
...
# Brush selection for interactive zooming/selection on panels
brush = alt.selection_interval()

# --- Filter widgets as separate charts to ensure separate rows and desired order ---
# Nation filter widget (first row)
nation_filter_widget = alt.Chart(df_analisi).mark_point().encode().add_params(
    nation_selection
).properties(title='Filtra per Nazione')

# Year filter widget (second row)
year_filter_widget = alt.Chart(df_analisi).mark_point().encode().add_params(
    year_selection
).properties(title='Filtra per Anno')

# Combine the filter widgets vertically into a column
...
# Base chart definition with common encodings and interactive elements (brush, color)
# Filters (year_selection, nation_selection) are applied via transform_filter here
base_chart_template = alt.Chart(df_analisi).mark_circle(size=60, opacity=0.6).encode(
    ...
    brush # Only brush is directly added to the data panels
).transform_filter(
    year_selection
).transform_filter(
    nation_selection
...
# Concatenate the main data panels horizontally
main_panels_hconcat = (panel_pil_pop | panel_pil_score | panel_host_score).resolve_scale(
    color='shared'
)

# Finally, combine the filter column with the main panels horizontally
...
* PIL medio: ~511 miliardi di USD
* PIL pro capite: ~14.510 USD
* Popolazione media: ~74 milioni
* Score Medaglie medio: ~18.41
* Osservazioni: 833
...
* PIL medio: ~7.4 miliardi di USD
* PIL pro capite: ~1.093 USD
* Popolazione media: ~13 milioni
* Score Medaglie medio: ~1.28
* Osservazioni: 1.102
...
* PIL medio: ~2 miliardi di USD
* PIL pro capite: ~11.771 USD
* Popolazione media: ~307 mila abitanti
* Score Medaglie medio: ~0.05
* Osservazioni: 693
...
Il modello presenta le seguenti performance sulla scala reale:

- **R² (test, scala reale): ≈ 0.71**
- **Test R² (scala log, diagnostico): ≈ 0.64**
- **Train R² (scala log, diagnostico): ≈ 0.93**

➡️ Interpretazione:

- L'introduzione delle feature di lag migliora sensibilmente la capacità esplicativa del modello rispetto alla versione senza storico (R² scala reale era ≈ 0.52)
...
Il gap tra Train R² e Test R² (scala log) resta presente ma è interpretabile alla luce della cross-validation (Cella 32), dove il punteggio medio sale e si stabilizza in modo molto più coerente con il test set rispetto alla versione precedente — segno che il miglioramento non è dovuto solo a un fit più aggressivo, ma a informazione realmente predittiva (lo storico recente della Nazione).
...
Come atteso, le feature di lag risultano tra le più importanti nel modello (si veda la Feature Importance in Cella 33): il punteggio dell'edizione precedente e la media mobile delle ultime edizioni sono fortemente predittivi, più ancora di alcune variabili macroeconomiche. Questo è coerente con la letteratura sportiva: i sistemi sportivi nazionali tendono a essere persistenti nel tempo (investimenti, infrastrutture e talenti non si costruiscono né si perdono da un'edizione all'altra).
...
Tra le nazioni che ottengono performance significativamente superiori rispetto alle attese emergono casi rilevanti, tra cui:

- **Federazione Russa (2004)** → +165.7 medaglie rispetto al previsto  
- **Cina (1996, 1984)** → deviazioni molto elevate  
- **Canada (1984)**  
- **Giappone (1964)**  
- **Ungheria (1980)**  
- **Cuba (2004)**  
...
Questi Paesi rappresentano esempi di **over-performance strutturale**, ovvero capacità di ottenere risultati molto superiori rispetto alle risorse economiche e demografiche.
...
- contesti geopolitici o storici particolari  

➡️ Caso emblematico:  
l'eccezionale performance della Cina e della Russia è compatibile con modelli di sviluppo sportivo altamente strutturati e orientati al risultato.
...
### 2. Under-performer: inefficienza relativa

Tra le nazioni con performance inferiori alle attese emergono:

- **Federazione Russa (2020)** → forte under-performance  
- **Regno Unito (1996, 1992)**  
- **Giappone (1980, 2000)**  
- **India (2004, 2016)**  
- **Francia (2020)**  
- **Brasile (1992)**  
...
Questi risultati indicano una **sotto-performance rispetto al potenziale strutturale**, suggerendo possibili inefficienze.

Le cause possono includere:

- scarsa organizzazione del sistema sportivo  
- investimenti insufficienti o non efficaci  
- inefficienze istituzionali  
- contesti specifici dell’edizione olimpica  

➡️ Caso particolare:
alcuni valori estremi (es. Russia 2020) possono essere influenzati da fattori esterni straordinari, come esclusioni o condizioni anomale.
...
👉 ma anche da fattori qualitativi e strategici  
...
➡️ Gli over-performer rappresentano i casi di alta efficienza  
➡️ Gli under-performer rappresentano inefficienze relative  
...
- evidenzia i limiti del modello  
...
➡️ i residui misurano la capacità reale di trasformarlo in successo sportivo  
...
L’identificazione degli outlier consente di isolare le osservazioni che presentano deviazioni estreme rispetto alle previsioni del modello, ovvero i casi in cui il residuo supera la soglia di ±2 deviazioni standard.
...
Tra gli outlier positivi emergono casi molto rilevanti:

- **Federazione Russa (2004)** → +165.7  
- **Cina (1996, 1984)**  
- **Stati Uniti (2016)**  
- **Ungheria (1980, 2012, 2016)**  
- **Cuba (1976, 2004)**  
- **Corea del Sud (1992, 1984, 2008)**  
- **Giappone (1964)**  
...
- contesti geopolitici (es. Guerra Fredda)  
...
Tra gli outlier negativi emergono:

- **Federazione Russa (2020)** → -58.0  
- **Regno Unito (1996)**  
- **Giappone (1980)**  
- **Italia (1988)**  
- **Francia (2020)**  
...
Questi casi rappresentano situazioni in cui il risultato reale è molto inferiore al potenziale stimato.

👉 Possibili cause:
- inefficienze strutturali  
- discontinuità nei programmi sportivi  
- fattori esterni (squalifiche, contesto storico, partecipazione ridotta)
...
- l’individuazione degli over/under performer  
- i limiti del modello Random Forest  

➡️ Questo conferma che:

👉 esiste una componente sistematica non osservata  
...
- isolano contesti in cui entrano in gioco fattori qualitativi  
- permettono di identificare modelli nazionali specifici  

➡️ In particolare:

- gli outlier positivi indicano **efficienza elevata**  
- gli outlier negativi indicano **inefficienza relativa**  
...
➡️ queste deviazioni non sono casuali, ma riflettono caratteristiche profonde non catturate dal modello  

Questo rafforza la conclusione generale del progetto:

👉 il successo olimpico non è interamente spiegabile da variabili economiche, ma dipende anche da fattori istituzionali, culturali e strategici difficilmente quantificabili.
...
**Nota:** i punteggi numerici riportati di seguito vanno verificati ri-eseguendo la cella nel proprio ambiente, poiché dipendono dal modello effettivamente addestrato (ora comprensivo delle feature di lag). In una verifica di pipeline equivalente, l'introduzione del lag porta il punteggio medio di cross-validation a salire sensibilmente rispetto alla versione precedente (era ≈ 0.38) e soprattutto a un gap molto più ridotto rispetto al Test R² in scala reale.
...
- **Score_Medaglie_Lag1_model → 0.597 (~59.7%)**

Seguita da:

- **Score_Medaglie_MediaMobile3_model → 0.190 (~19.0%)**

Insieme le due feature di lag coprono quasi il 79% dell'importanza totale del modello. Questo risultato dice che il predittore più forte del risultato olimpico di una nazione in una certa edizione è semplicemente **quanto ha fatto bene nelle edizioni precedenti**: i sistemi sportivi nazionali hanno una forte inerzia nel tempo.
...
- **Popolazione_Urbana → ~3.5%**
- **Aspettativa_di_Vita → ~2.9%**
- **Iscrizioni_Scuola_Primaria_perc → ~2.5%**
- **Pop_Log → ~2.0%**
- **Anno → ~1.7%**
...
- **Ha_Storico_Precedente → ~0.4%**: il flag che distingue le prime partecipazioni ha un contributo minimo — il modello riesce a gestirlo principalmente attraverso il valore del lag (= 0 per le prime partecipazioni).
...
**Distribuzione dei residui** (N=519 osservazioni di test):

- media ≈ 1.64
- mediana ≈ -0.06
- deviazione standard ≈ 10.16
- minimo ≈ -90.65, massimo ≈ +83.26

La mediana vicina allo zero dice che il modello è ben calibrato sulla maggior parte dei casi. La media leggermente positiva indica una lieve tendenza a sottostimare più spesso di quanto sovrastimi. Il divario tra la deviazione standard (10.16) e i valori estremi mostra che ci sono pochi casi molto lontani dalla media, coerente col fatto che ci sono poche "superpotenze" olimpiche e molte nazioni minori.

Dal grafico dei residui e dall'istogramma si vede una forte concentrazione intorno allo zero, con outlier sia positivi che negativi di entità comparabile. Il modello quindi è accurato nella maggior parte dei casi, ma fatica un po' a cogliere le performance più estreme in entrambe le direzioni.

**Residui per cluster socio-economico** (ora raggruppati per `Cluster_Label`, non più per il numero grezzo del cluster K-Means, per evitare l'ambiguità di etichettatura già segnalata altrove nel progetto):

- Nazioni Minori: -0.05
- Economie Emergenti: +1.54
- Grandi Potenze Olimpiche: +3.10

Le Grandi Potenze hanno il residuo medio più alto: il modello tende a sottostimarle leggermente più delle altre categorie, probabilmente perché è proprio lì che si concentrano i casi più estremi (host, boicottaggi, exploit). Le Nazioni Minori hanno un residuo medio praticamente nullo: per loro il modello è già ben calibrato di suo.

**Casi estremi** (dalla tabella Top 5 / Bottom 5 sopra):

- maggiori over-performer: Germany (+83.3), Canada (+78.4), China (+68.4), Great Britain (+56.1), Japan (+53.9)
- maggiori under-performer: Russian Federation (-90.7), Germany (-58.1), Japan (-19.2), Australia (-18.9), Italy (-17.8)

Questi scostamenti non sono spiegabili con le sole variabili economiche: performance eccezionali (Canada, Cina) o cali improvvisi (Russia, per motivi non economici come squalifiche o boicottaggi in specifiche edizioni). Le cause specifiche restano ipotesi qualitative coerenti con la letteratura, non risultati diretti del modello — il Random Forest individua che il residuo esiste, non da cosa dipende. Da notare che Germany e Japan compaiono sia tra gli over- che tra gli under-performer: lo stesso Paese può avere edizioni molto sopra e molto sotto le attese a seconda del contesto storico specifico.

In generale, il modello coglie bene la struttura di fondo del fenomeno, ma non i casi estremi in nessuna delle due direzioni. Il messaggio di fondo resta quello del progetto: esiste un potenziale economico teorico, ma quanto viene effettivamente realizzato dipende da fattori che l'economia da sola non cattura.
...
C'era una discrepanza tra il valore di CV usato nei documenti (0.671) e quello che esce davvero dalla cella di cross-validation. Rifaccio lo stesso identico calcolo per capire quale dei due è corretto.
"""
...
print()
print("Se questo numero è ~0.655, il valore '0.671' citato nel report/ripasso/PPTX era una")
print("confusione con il Test R² (scala log) dello split singolo — due metriche distinte.")
...
print(f"R² (scala log, per confronto con Test R² log = 0.671 del modello originale): {r2_temp_log:.3f}")
print()
print("Confronto con il modello originale (split casuale): R²=0.682 (scala reale).")
print("Questo è il numero di riferimento da citare per lo split temporale (sostituisce")
print("qualunque altro valore calcolato con un sottoinsieme di righe diverso).")
...
la **dimensione economica complessiva (PIL)** rappresenta il principale driver del successo olimpico.
...
Il passaggio dalla regressione lineare semplice (R² ≈ 0.19, Cella 17) al Random Forest con feature di lag (R² = 0.682 in scala reale, Cella 27) dimostra che:
...
Nonostante le buone performance del Random Forest (R² = 0.682 in scala reale, cioè il 68% della variabilità spiegata), circa il **32% della variabilità rimane non spiegato** dal modello.  
...
Il 32% residuo non è spiegato dal modello. L'attribuzione a politiche sportive, organizzazione istituzionale o tradizioni è un'**ipotesi interpretativa** — la varianza residua è evidenza statistica, le sue cause possibili sono interpretazione.
...
> **Il PIL determina il potenziale olimpico strutturale di una nazione nel lungo periodo. L'inerzia del sistema sportivo — misurata dallo storico recente — ne governa la performance nel breve. I residui rivelano dove efficienza, scelte politiche e fattori non misurabili fanno la differenza tra realizzare quel potenziale o sprecarlo.**
...
Il successo olimpico nasce dall'interazione tra risorse materiali, inerzia storica della performance sportiva e capacità di convertire quel potenziale in risultati concreti.

In altre parole:

👉 l'economia definisce il potenziale strutturale  
👉 lo storico recente quantifica quanto quel potenziale è già stato attivato  
👉 il risultato finale dipende da scelte strategiche, istituzioni e fattori che i dati disponibili non riescono a catturare

**Aggiornamento rispetto alla proposta progettuale:** l'introduzione delle variabili di lag (`Score_Medaglie_Lag1`, `Score_Medaglie_MediaMobile3`), inizialmente prevista come sviluppo futuro, è stata implementata in questa versione del notebook. I risultati confermano che il trend storico della performance è uno dei predittori più rilevanti del modello, migliorando sia l'accuratezza sul test set sia la stabilità in cross-validation rispetto alla versione basata solo su variabili macroeconomiche statiche.



---

### Controlli aggiuntivi (Celle 39-46)

Per rispondere a due possibili obiezioni metodologiche, ho aggiunto qualche controllo in più:

- **Il numero di cross-validation era sbagliato**: il valore giusto è 0.655 (Cella 39), non 0.671 come scritto nei documenti — quel numero era in realtà il Test R² in scala log, una metrica diversa.
- **Nazioni condivise tra train e test**: con lo split casuale originale il 100% delle nazioni del test set compare anche nel training set in un'altra edizione (Cella 40). Con `GroupKFold` per nazione (Cella 41) l'R² scende da 0.655 a 0.640 — un calo contenuto, non un crollo.
- **Split temporale**: allenando solo su edizioni fino al 2008 e testando su 2012-2020 (mai viste), il modello ottiene R²=0.864 in scala reale (Cella 43), persino meglio dello split casuale. Il vantaggio regge anche togliendo le feature di lag (0.675 invece di 0.864, Cella 44) e non dipende da una singola edizione (2012: 0.93, 2016: 0.86, 2020: 0.79, Cella 45). Va detto però che questo test non copre il caso di una nazione debuttante, perché nel 2012-2020 tutte le nazioni avevano già partecipato prima (Cella 46).
- **Iperparametri**: una ricerca con `RandomizedSearchCV` (Cella 42) trova una combinazione leggermente migliore (max_depth=6, min_samples_leaf=2) con R²=0.657 contro 0.640 — un guadagno piccolo, quindi la scelta originale (max_depth=10) non era irragionevole.

Nel complesso questi controlli confermano che il modello regge, con un margine di leakage da nazioni condivise contenuto ma non nullo, e un'ottima capacità di generalizzare anche su edizioni future.
"""