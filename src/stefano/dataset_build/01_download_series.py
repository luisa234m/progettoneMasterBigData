...
INDICATORS = str(PROJECT_ROOT) + "/data/csv_wb_paesi"
...
def filter_valid_economies(codes:list, countries:list, warning=True):
    ...
    """
    valid = []
    file_path = INDICATORS +"/valid.txt"
...
def list_olympic_countries(warning:bool=False):
    ...
    """
    encoding = utils.detect_encoding("ISO.csv")
    df = pd.read_csv("ISO.csv", encoding=encoding)
...
def main():
    ...
    df = pd.read_csv("indicators.csv", skipinitialspace=True, sep=";")
